...
namespace AutomaticScheduleLoader
{
    ...
    (
        ...
        public void PopulateGridViewActivtyLog() 
        {
            using (SqlConnection sqlcon = new SqlConnection(conn))
            {
                sqlcon.Open();
                SqlDataAdapter adapter = new SqlDataAdapter("SELECT * FROM ActivityLog_Tbl", conn);
                ...
                this.dataGridView1.Columns["ActivityLog"].HeaderText = "Activity Log";
                this.dataGridView1.Columns["DateTime"].HeaderText = "Date";
...
dataGridView1.Height, 5, 5));
        ...
        {
            DialogResult dr = MessageBox.Show("Do you really want to clear activity log history?","Clear history",MessageBoxButtons.YesNo);
            if (dr == DialogResult.Yes)
            {
                using (SqlConnection sqlcon = new SqlConnection(conn))
                {
                    sqlcon.Open();
                    SqlCommand cmdDelAll = new SqlCommand("TRUNCATE TABLE ActivityLog_Tbl", sqlcon);
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            {
                sqlcon.Open();
                SqlDataAdapter adapter = new SqlDataAdapter("SELECT * FROM ActivityLog_Tbl where DateTime between '" + dateTimePicker1.Value.ToString(format) + "' and '" + dateTimePicker2.Value.ToString(format) + "'", conn);
                DataTable dt = new DataTable();
                adapter.Fill(dt);
                dataGridView1.DataSource = dt;
                dataGridView1.EnableHeadersVisualStyles = false;
                ...
                this.dataGridView1.Columns[1].Width = 170;
                this.dataGridView1.Columns[2].AutoSizeMode = DataGridViewAutoSizeColumnMode.Fill;
                dataGridView1.Columns["ActivityLog"].HeaderText = "Activity Log";
                this.dataGridView1.Columns["DateTime"].HeaderText = "Date";
                dataGridView1.Sort(dataGridView1.Columns[1], ListSortDirection.Descending);
                this.dataGridView1.Columns[0].ReadOnly = true;
                this.dataGridView1.Columns[1].ReadOnly = true;
